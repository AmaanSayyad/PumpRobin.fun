...
/** Set NEXT_PUBLIC_TELEGRAM_URL when ready (e.g. https://t.me/yourchannel) */
...
const RESOURCES = [
  ...
  ...(TELEGRAM_URL
    ? [{ href: TELEGRAM_URL, label: "Telegram", external: true as const }]
    : []),
];
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-6 mb-10">
          ...
          <div className="flex items-center gap-4">
            ...
            <div className="flex items-center gap-3.5">
              {SOCIAL.map((s) => {
                const ready = "ready" in s ? s.ready !== false : true;
                if (!ready) {
                  return (
                    <span
                      key={s.label}
                      ...
                      className="text-rh-on-lime/50 cursor-default"
                    >
                      {s.icon}
                    </span>
                  );
